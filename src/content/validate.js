...
export const WARNING_RULES = Object.freeze(["wording_length", "question_fields"]);
...
export function validateDeck(input) {
  ...
  if (!Number.isInteger(input["releaseSeq"])) {
    fail("release_seq", `releaseSeq must be an integer, got ${JSON.stringify(input["releaseSeq"])}`);
  }
  ...
  questions.forEach((raw, index) => {
    ...
    for (const key of Object.keys(raw)) {
      if (!QUESTION_FIELDS.includes(key)) warn("question_fields", `${label}: unknown field "${key}" ignored`);
    }
    ...
    for (const { lang, value, seen } of wordings) {
      ...
      if (HTML_TAG.test(value)) fail("no_html", `${label}: ${lang} contains an HTML tag`);
  ...
  return { ok, errors, warnings, deck: ok ? /** @type {Deck} */ (/** @type {unknown} */ (input)) : null };
}
...
export function formatReport(result) {
  ...
  const extraWarnings = result.warnings.filter((f) => !RULES.includes(f.rule));
  if (extraWarnings.length > 0) {
    lines.push("WARN  other");
    for (const f of extraWarnings) lines.push(`        - ${f.message}`);
  }
  lines.push(
    result.ok
