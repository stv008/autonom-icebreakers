...
Not verifiable in the build environment (the embedded review browser refuses service-worker script fetches): service-worker registration, offline launch in airplane mode, the code-update banner path, "Available offline" turning true, and Space/Enter on a focused button (the automation cannot trigger native button activation; the handler is code-verified to ignore button targets). These need a run in desktop Chrome/Safari over `localhost` or HTTPS and on a phone.
