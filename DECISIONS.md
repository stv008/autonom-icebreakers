...
## Logo on the cards (2026-09-26) — v0.1.3

49. **Every card carries the Autonom logo, on CEO request** ("integrate the Autonom logo on each card"). Three brand-safe options were rendered and put to the CEO: (A) all-white logo, (B) the unchanged master logo on a white label, (C) white wordmark with the four-colour mark. **The CEO chose C.** The existing dark variant could not be used as-is: the mark's deep-blue stroke (`#0903a4`) almost vanishes on the `#10069F` card. `public/logo-autonom-card.svg` is therefore `logo-autonom-dark.svg` with that one stroke set to white; the other three mark colours and every path are unchanged. It is a **second derived variant** and belongs next to decision 43 in `Context/brand/exceptii-registru.md` once Marketing confirms it (or supplies an official negative logo, which then replaces both derived files).
50. **Placement and size, as chosen by the CEO:** bottom-left, opposite the star; discreet (1.375 rem tall on the card, 1.75 rem in present mode, where the top-bar logo is hidden and the card logo is the only branding on the shared screen). Also shown on notice cards and the load-error card. `alt=""`: the logo is decorative there, and the top bar already names the brand for screen readers. Present mode reserves 4 rem below the text so a long question never runs into it.
